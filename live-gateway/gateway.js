...
const { getCurrentServerConfig, printConfig, USE_MOCK_SERVER, LOCAL_SERVER_URL } = serverCfg;

const currentConfig = getCurrentServerConfig();
const port = currentConfig.port;

// 后端服务地址（Spring Boot）
const BACKEND_URL = LOCAL_SERVER_URL || 'http://localhost:8080';
...
// 投票数据存储
let voteData = {
    leftVotes: Math.floor(Math.random() * 500),
    rightVotes: Math.floor(Math.random() * 500),
    leftPercentage: 50,
    rightPercentage: 50
};
// 初始化百分比
const initTotal = voteData.leftVotes + voteData.rightVotes;
if (initTotal > 0) {
    voteData.leftPercentage = Math.round((voteData.leftVotes / initTotal) * 100);
    voteData.rightPercentage = 100 - voteData.leftPercentage;
}

// 弹幕数据存储
let danmakuData = [];
const danmakuTemplates = [
    '支持正方！', '支持反方！', '说得好！', '太精彩了！', '加油！',
    '这个观点很棒', '我同意！', '不同意这个说法', '有道理', '继续辩论！',
    '666', '👏', '👍', '辩论很激烈', '期待下一轮',
    '双方都很厉害', '学到了', '受益匪浅', '精彩！', '支持辩论！'
];
const userNames = ['观众_小明', '观众_小红', '观众_阿强', '观众_小美', '观众_大壮', '观众_小丽', '观众_老王', '观众_小张'];

// 模拟弹幕生成器
function generateDanmaku() {
    const content = danmakuTemplates[Math.floor(Math.random() * danmakuTemplates.length)];
...
let danmakuTimer = setInterval(() => {
    if (wsClients.size > 0 && Math.random() > 0.3) {
        const danmaku = generateDanmaku();
        danmakuData.push(danmaku);
        if (danmakuData.length > 100) {
            danmakuData.shift();
        }
        broadcast('danmaku', danmaku);
    }
}, 2000);
...
if (!USE_MOCK_SERVER) {
    app.use('/api', createProxyMiddleware({
        ...
        onProxyReq: (proxyReq, req, res) => {
            console.log(`🔄 代理请求: ${req.method} ${req.path} -> ${BACKEND_URL}${req.path}`);
        },
        onError: (err, req, res) => {
            console.error('❌ 代理错误:', err);
            res.status(503).json({ error: '后端服务不可用' });
        }
    }));
    console.log(`🚀 已配置代理: /api -> ${BACKEND_URL}`);
} else {
    console.log(`⚠️  使用本地Mock模式，不启用代理`);
}
...
app.get('/api/v1/admin/streams/:id', (req, res) => {
    try {
        const db = require('./admin/db.js');
        ...
        res.status(500).json({
            success: false,
            message: '获取直播流失败: ' + error.message
        });
    }
});

app.post('/api/v1/admin/streams', (req, res) => {
    try {
        const db = require('./admin/db.js');
        const { name, url, enabled = false } = req.body;
        
        if (!name || !url) {
            return res.status(400).json({
                success: false,
                message: '缺少必要参数: name 和 url'
            });
        }
        
        const newStream = db.streams.create({
            name,
            url,
            enabled
        });
        ...
        res.status(500).json({
            success: false,
            message: '创建直播流失败: ' + error.message
        });
    }
});

app.put('/api/v1/admin/streams/:id', (req, res) => {
    try {
        const db = require('./admin/db.js');
        const { id } = req.params;
        const { name, url, enabled } = req.body;
        
        const updated = db.streams.update(id, { name, url, enabled });
        if (!updated) {
            return res.status(404).json({
                success: false,
                message: '直播流不存在'
...
app.delete('/api/v1/admin/streams/:id', (req, res) => {
    try {
        const db = require('./admin/db.js');
        const { id } = req.params;
        
        const deleted = db.streams.delete(id);
        if (!deleted) {
            return res.status(404).json({
                success: false,
                message: '直播流不存在'
...
app.get('/api/v1/admin/dashboard', (req, res) => {
    try {
        const streamId = req.query.stream_id || 'stream-001';
        ...
        const dashboardData = {
            ...
            peakViewers: Math.floor(Math.random() * 2000) + 1000,
            ...
            leftPercentage: 50,
            rightPercentage: 50,
            streamUrl: `rtmp://localhost:1935/live/${streamId}`,
            hlsUrl: `http://localhost:3001/hls/${streamId}/index.m3u8`,
            createdAt: Date.now(),
            debateTopic: {
                title: '如果有一个能一键消除痛苦的按钮，你会按吗？',
                description: '这是一个关于痛苦、成长与人性选择的深度辩论',
                leftSide: '正方观点：应该按，追求幸福是人类天性',
                rightSide: '反方观点：不应该按，痛苦使人成长'
            }
        };
        
        // 计算投票百分比
        const totalVotes = dashboardData.leftVotes + dashboardData.rightVotes;
        if (totalVotes > 0) {
            dashboardData.leftPercentage = Math.round((dashboardData.leftVotes / totalVotes) * 100);
            dashboardData.rightPercentage = 100 - dashboardData.leftPercentage;
        }
    ...
    try {
        ...
        res.json({
            ...
            data: {
                ...
                totalVotes: voteData.leftVotes + voteData.rightVotes,
                leftPercentage: voteData.leftPercentage,
                rightPercentage: voteData.rightPercentage,
                updatedAt: Date.now()
            }
        });
    } catch (error) {
...
app.post('/api/v1/user-vote', (req, res) => {
    try {
        const { stream_id, voteType, user_id, leftVotes, rightVotes } = req.body;
        
        // 支持两种格式：
        // 格式1（增量投票）: { voteType: "left"|"right" }
        // 格式2（100票分配）: { leftVotes: number, rightVotes: number }
        if (voteType && (voteType === 'left' || voteType === 'right')) {
            // 格式1：增量投票
            if (voteType === 'left') {
                voteData.leftVotes++;
            } else {
        ...
        } else if (leftVotes !== undefined || rightVotes !== undefined) {
            // 格式2：100票分配
            const left = parseInt(leftVotes) || 0;
            const right = parseInt(rightVotes) || 0;
            voteData.leftVotes += left;
            voteData.rightVotes += right;
        } else {
            return res.status(400).json({
                success: false,
                message: '无效的投票类型'
        ...
        const totalVotes = voteData.leftVotes + voteData.rightVotes;
        voteData.leftPercentage = totalVotes > 0 ? Math.round((voteData.leftVotes / totalVotes) * 100) : 50;
        voteData.rightPercentage = 100 - voteData.leftPercentage;
        
        res.json({
            success: true,
            data: {
                streamId: stream_id || 'stream-001',
                voteType: voteType,
                leftVotes: voteData.leftVotes,
                rightVotes: voteData.rightVotes,
                totalVotes: totalVotes,
...
app.get('/api/v1/ai-content', (req, res) => {
    try {
        ...
        let content = [...aiDebateContent];
        
        if (streamId) {
            content = content.filter(item => item.streamId === streamId);
        }
        ...
        res.status(500).json({
            success: false,
            message: '获取AI内容失败: ' + error.message
        });
    }
});
...
app.get('/api/v1/user-votes', (req, res) => {
    try {
        ...
        res.json({
            success: true,
            data: {
                userId: user_id || 'user-001',
                streamId: stream_id || 'stream-001',
                votes: [
                    { id: 'vote-001', voteType: 'left', createdAt: Date.now() - 3600000 },
                    { id: 'vote-002', voteType: 'right', createdAt: Date.now() - 1800000 }
                ]
            }
        });
    } catch (error) {
        ...
        res.status(500).json({
            success: false,
            message: '获取用户投票记录失败: ' + error.message
        });
    }
});
    ...
    try {
        ...
        res.json({
            ...
            data: {
                ...
                rightPercentage: voteData.rightPercentage,
                trend: [
                    { time: '10:00', leftVotes: 100, rightVotes: 80 },
                    { time: '10:10', leftVotes: 150, rightVotes: 120 },
                    { time: '10:20', leftVotes: 200, rightVotes: 180 },
                    { time: '10:30', leftVotes: 250, rightVotes: 220 },
                    { time: '10:40', leftVotes: 300, rightVotes: 280 }
                ],
                updatedAt: Date.now()
            }
        });
...
app.post('/api/v1/admin/live/update-votes', (req, res) => {
    try {
        const { stream_id, left_votes, right_votes } = req.body;
        
        if (left_votes !== undefined) voteData.leftVotes = parseInt(left_votes);
        if (right_votes !== undefined) voteData.rightVotes = parseInt(right_votes);
...
app.post('/api/v1/admin/live/reset-votes', (req, res) => {
    try {
        ...
        voteData.leftVotes = 0;
        voteData.rightVotes = 0;
        voteData.leftPercentage = 50;
        voteData.rightPercentage = 50;
...
app.post('/api/v1/admin/ai/stop', (req, res) => {
    try {
        ...
        console.error('停止AI失败:', error);
        res.status(500).json({
            success: false,
            message: '停止AI失败: ' + error.message
        });
    }
});
...
app.get('/api/v1/admin/users', (req, res) => {
    try {
        res.json({
            success: true,
            data: [
                { id: 'user-001', nickName: '测试用户_001', avatarUrl: '/static/iconfont/blue-user.png', role: 'viewer' },
                { id: 'user-002', nickName: '测试用户_002', avatarUrl: '/static/iconfont/green-user.png', role: 'viewer' },
                { id: 'user-003', nickName: '测试用户_003', avatarUrl: '/static/iconfont/yellow-user.png', role: 'admin' },
                { id: 'user-004', nickName: '测试用户_004', avatarUrl: '/static/iconfont/red-user.png', role: 'judge' }
            ]
        });
    } catch (error) {
        console.error('获取用户列表失败:', error);
    ...
    try {
        ...
        res.json({
            ...
            data: {
                ...
                viewerCount: Math.floor(Math.random() * 1000) + 500,
                viewers: [
                    { id: 'viewer-001', nickName: '观众_001', avatarUrl: '/static/iconfont/blue-user.png', joinedAt: Date.now() - 3600000 },
                    { id: 'viewer-002', nickName: '观众_002', avatarUrl: '/static/iconfont/green-user.png', joinedAt: Date.now() - 1800000 },
                    { id: 'viewer-003', nickName: '观众_003', avatarUrl: '/static/iconfont/yellow-user.png', joinedAt: Date.now() - 900000 }
                ]
            }
        });
    } catch (error) {
        ...
        res.status(500).json({
            success: false,
            message: '获取观众列表失败: ' + error.message
        });
    }
});

// v1: 更新观众列表
app.post('/api/v1/admin/live/viewers', (req, res) => {
    try {
        ...
        res.json({
            ...
            data: {
                streamId: stream_id || 'stream-001',
                viewerCount: viewer_count || Math.floor(Math.random() * 1000) + 500,
                message: '观众数已更新'
            }
        });
    } catch (error) {
        console.error('更新观众列表失败:', error);
        res.status(500).json({
            success: false,
            message: '更新观众列表失败: ' + error.message
        });
    }
});
    ...
    try {
        ...
        res.json({
            ...
            data: {
                ...
                peakViewers: Math.floor(Math.random() * 3000) + 1500,
                platforms: [
                    { name: '抖音', viewers: Math.floor(Math.random() * 2000) + 500 },
                    { name: '快手', viewers: Math.floor(Math.random() * 1500) + 300 },
                    { name: 'B站', viewers: Math.floor(Math.random() * 1000) + 200 },
                    { name: '视频号', viewers: Math.floor(Math.random() * 500) + 100 }
                ]
            }
        });
    } catch (error) {
        console.error('获取广播观众失败:', error);
        res.status(500).json({
            success: false,
            message: '获取广播观众失败: ' + error.message
        });
    }
});

// v1: 获取投票信息（非v1版本，兼容旧版前端）
app.get('/api/votes', (req, res) => {
    try {
        const streamId = req.query.stream_id || 'stream-001';
        
        const leftVotes = Math.floor(Math.random() * 500);
        const rightVotes = Math.floor(Math.random() * 500);
...
app.get('/api/admin/rtmp/urls', (req, res) => {
    try {
        const roomName = req.query.room_name || 'stream-001';
        
        res.json({
            success: true,
            data: {
                room_name: roomName,
                push_url: `rtmp://localhost:1935/live/${roomName}`,
                play_flv: `http://localhost:3001/flv/${roomName}.flv`,
                play_hls: `http://localhost:3001/hls/${roomName}/index.m3u8`,
                rtmp_url: `rtmp://localhost:1935/live/${roomName}`,
                hls_url: `http://localhost:3001/hls/${roomName}/index.m3u8`,
                flv_url: `http://localhost:3001/flv/${roomName}.flv`,
                ws_url: `ws://localhost:3001/ws/${roomName}`
            }
        });
    } catch (error) {
        console.error('获取RTMP URL失败:', error);
        res.status(500).json({
            success: false,
            message: '获取RTMP URL失败: ' + error.message
        });
    }
});

// v1: 获取RTMP转HLS URL（v1版本）
...
// v1: 进入直播间
app.post('/api/v1/live/enter', (req, res) => {
    try {
        const { stream_id, user_id, user_name, avatar_url } = req.body;
        ...
        res.json({
            ...
            data: {
                streamId: stream_id || 'stream-001',
                userId: user_id || 'user-001',
                userName: user_name || '测试用户',
                avatarUrl: avatar_url || '/static/iconfont/blue-user.png',
    ...
    try {
        ...
        res.json({
            ...
            data: {
                ...
                leftSide: debateTopic.leftSide || '正方观点',
                rightSide: debateTopic.rightSide || '反方观点',
                createdAt: Date.now()
            }
        });
    } catch (error) {
